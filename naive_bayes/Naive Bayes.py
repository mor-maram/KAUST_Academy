...
def get_word_frequency(X, Y):
    ...
    for i in range(len(X)):
        # Get the i-th email
        email = X[i]
        # Get the i-th label. This indicates whether the email is spam or not. 1 = spam, 0 = ham
        cls = Y[i]
        ...
        for word in email:
            # If the word is not already in the dictionary, initialize its count
            if word not in word_dict:
                word_dict[word] = {"spam": 1, "ham": 1}
            
            # Increment the count in the appropriate category
            if cls == 0:
...
def prob_word_given_class(word, cls, word_frequency, class_frequency):
    ...
    total_words_in_class = class_frequency[cls]
    p_word_given_class = amount_word_and_class / total_words_in_class if total_words_in_class > 0 else 0
...
def prob_email_given_class(treated_email, cls, word_frequency, class_frequency):
    ...
    for word in treated_email:
        ...
        if word in word_frequency.keys(): 
            ...
            word_count_in_class = word_frequency[word].get(cls, 0)
            total_words_in_class = class_frequency.get(cls, 1)
            p_word_given_class = word_count_in_class / total_words_in_class
            prob *= p_word_given_class
